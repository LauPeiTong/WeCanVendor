...
export default {
  ...
  mounted () {
    ...
    this.nutrients = this.product.nutrients
    this.price = this.product.original_price
    this.discount = this.product.discount
    this.quantity = this.product.quantity
    this.expiredDate = (new Date(new Date(this.product.expired_date) - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10)
  },
  methods: {
    goBackToProductPage () {
      this.$router.push({ name: 'products-productId', params: { productId: this.id, product: this.product } })
    },
    async addItem () {
      try {
        ...
        const product = await this.$axios.put(`/api/products/${this.id}/`, newProductData)
        console.log('New product:', product)

        this.$router.push('/products')
      } catch (e) {
        console.log('Fail to add item: ', e)
      }
    },
    async deleteItem () {
      try {
        const response = await this.$axios.delete(`/api/products/${this.id}/`)
